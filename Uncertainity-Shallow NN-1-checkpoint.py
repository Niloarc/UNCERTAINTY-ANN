...
# Assuming the MFSP values are in the last column and the input variables in the others
X = df.iloc[:, :-1]  # Input features
y = df.iloc[:, -1]   # MFSP (Target)
...
from scikeras.wrappers import KerasRegressor  # Ensure you use scikeras for Keras integration

# Define hyperparameter space
param_distribs = {
    'model__n_hidden': [1, 2, 3],  # Increased hidden layers for optimization
    'model__n_neurons': [30, 50, 100],  # Number of neurons per layer
    'model__activation_fun': ['relu', 'tanh', 'elu'],  # Activation functions
    'model__neural_optimizer': ['adam', 'rmsprop', 'sgd'],  # Optimizers
    'model__learning_rate': [0.01, 0.001, 0.0001],  # Learning rates
...
keras_reg = KerasRegressor(
    model=build_model,  # The model-building function
    verbose=0
)
...
# Retrieve and print the best parameters and estimator
...
# Retrieve the best parameters and save to a variable
...
def plot_nn_learning_curve(X_train, X_test, y_train, y_test, model, increments=10):
    ...
    for size in train_sizes:
        ...
        train_scores.append(r2_score(y_train_subset, y_train_pred))  # R² score for training

        # Evaluate performance on the test set
        y_test_pred = model.predict(X_test).ravel()
        test_scores.append(r2_score(y_test, y_test_pred))  # R² score for testing
...
# Call the function for the neural network
